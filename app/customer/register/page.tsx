...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/customer/register', {
      ...
      body: JSON.stringify({ email, password, licenseKey: licenseKey || undefined }),
    });
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-8">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div>
              <label className="text-zinc-400 text-xs mb-1.5 block">
                License Key <span className="text-zinc-600">(optional — link your purchase)</span>
              </label>
              <input
                type="text" value={licenseKey} onChange={(e) => setLicenseKey(e.target.value)}
                className="w-full bg-white/5 border border-white/10 text-white text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-indigo-500 placeholder:text-zinc-600 font-mono"
                placeholder="FTWAC-XXXX-XXXX-XX-XX"
              />
            </div>
          ...
          <p className="text-zinc-600 text-xs text-center mt-5">
            ...
            <Link href="/customer/login" className="text-indigo-400 hover:text-indigo-300">Sign in</Link>
          </p>
        </div>
